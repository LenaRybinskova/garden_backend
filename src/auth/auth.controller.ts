...
export class AuthController {
  ...
  @ApiBody({
    type: RegisterDto,
    description: 'Необходимые данные для регистрации нового пользователя:',
  })
  ...
  @ApiBody({
    type: LoginDto,
    description: 'Необходимые данные для авторизации пользователя:',
  })
  @ApiResponse({ status: HttpStatus.OK, type: AccessTokenResponseDto })
  @ApiResponse({
  ...
  @ApiResponse({
    ...
    description: 'Проблемы с refresh token, нет такого пользователя',
    example: {
      statusCode: 401,
  ...
  @ApiOperation({
    ...
    description: 'В заголовке укажите Authorization: Bearer AccessToken',
  })
