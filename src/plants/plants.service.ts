...
export class PlantsService {
  ...
  async isOwnPlant(plantId: string, userId: string) {
    try {
      ...
      throw new ForbiddenException(
        'этот пользователь не может вносить изменения тк не является создателем Plant',
      );
    }
  }
  ...
  async create(dto: CreatePlantDto, user: User) {
    ...
    // проверяю, если такой Сезон у Юзера уже есть, то в него создаем Плант. Если передан сезон, которого его нет у Юзера, то создается новый сезон и к нему будет Плант относится.
    let currentSeason;
    if (dto.season) {
  ...
  async update(dto: UpdatePlantDto & { plantId: string; user: User }) {
    const currentUserIsOwnerPlant = await this.isOwnPlant(
      dto.plantId,
      dto.user.id,
    );
